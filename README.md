...
# 📁 Project Structure
.
├── images/                                                     # Chessboard images for offline calibration
│   ├── left01.jpg
│   ├── left02.jpg
│   ├── left03.jpg
│   ├── left04.jpg
...
│   ├── left08.jpg
│   └── left09.jpg
├── camera_calib.npz                                            # Saved camera intrinsics & distortion coefficients
├── camera_calibration_and_realtime.py                          # Python script for calibration + realtime undistortion
├── OpenCV-Camera-Calibration-and-Lens-Distortion.ipynb         # Full tutorial notebook (theory + experiments)
├── README.md                                                   # Project documentation
└── tools/                                                      # Utility module
    ├── tools.py                                                # Helper functions for visualization & processing
    └── __pycache__/                                            # Python bytecode cache (not required for version control)
...
The fundamental equation that maps a `3D` world point to a `2D` image point (_before distortion_):
...
Where:
- $\mathbf{K}$ → **camera intrinsic matrix** (_focal lengths + principal point_)
- $[\mathbf{R} \mid \mathbf{t}]$ → **extrinsic parameters** (_rotation matrix $\mathbf{R}$ + translation vector $\mathbf{t}$_)
- $s$ → arbitrary positive **scale factor** (_homogeneous coordinate normalization_)
- $(X, Y, Z)$ → `3D` point in world coordinates
- $(u, v)$ → corresponding `2D` image coordinates (_in pixels_)

#### 2. Lens Distortion Model (OpenCV / Brown–Conrady)

Real cameras introduce non-linear distortions. The most widely used model is:

- **Radial distortion** (_barrel or pincushion effect_):
...
- **Tangential distortion** (_decentering / lens misalignment_):
...
Where:
- $r^2 = x^2 + y^2$ (_normalized coordinates_)
...
- **Perform metric measurements** from images (_with known scale_)
...
- $\mathbf{m}_i$ → observed (_detected_) image point
...
- **Good** → **0.5–0.8 pixels**
- **> 1 pixel** → usually indicates problems (_poor images, inaccurate corners, insufficient views, or unmodeled distortion_)
...
- Advanced calibration (_fisheye, stereo_) enables wide-angle and multi-camera applications.
